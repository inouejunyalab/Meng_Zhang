...
// define the function in class, rotation along x,y,z directions according to the angle
void CRY_BOX::rot_xyz(double rot_angle[3]) {
	for (int bi = 0; bi < 3; bi++) {														// rotating along three directions

		double ang = rot_angle[bi] * PI / 180;
		double unit[3][3];
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++)
				unit[i][j] = xyz_vect[i][j];
		}

		int f = bi;
		for (int i = 0; i < 3; i++) {
			if (i == f)	continue;
			double tvect[3] = { 0,0,0 };
			double dot = unit[f][0] * unit[i][0] + unit[f][1] * unit[i][1] + unit[f][2] * unit[i][2];

			tvect[0] = unit[i][0] * cos(ang) + (unit[f][1] * unit[i][2] - unit[f][2] * unit[i][1]) * sin(ang) + unit[f][0] * dot * (1 - cos(ang));
			tvect[1] = unit[i][1] * cos(ang) + (-unit[f][0] * unit[i][2] + unit[f][2] * unit[i][0]) * sin(ang) + unit[f][1] * dot * (1 - cos(ang));
			tvect[2] = unit[i][2] * cos(ang) + (unit[f][0] * unit[i][1] - unit[f][1] * unit[i][0]) * sin(ang) + unit[f][2] * dot * (1 - cos(ang));
			if (fabs(tvect[0]) < 1.0e-5)
				tvect[0] = 0.0;
			if (fabs(tvect[1]) < 1.0e-5)
				tvect[1] = 0.0;
			if (fabs(tvect[2]) < 1.0e-5)
				tvect[2] = 0.0;

			xyz_vect[i][0] = tvect[0];
			xyz_vect[i][1] = tvect[1];
			xyz_vect[i][2] = tvect[2];
		}
	}
}
