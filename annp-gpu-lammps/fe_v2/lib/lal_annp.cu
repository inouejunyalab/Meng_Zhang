...
__kernel void k_annp(const __global numtyp4* restrict x_, const int ntypes,
	...
	if (ii < inum) {
		...
		numtyp hidly_d[10] = { 0.0 };																	
		numtyp lays_dw[10 * 28] = { 0.0 };																
		numtyp temp_dw[10 * 28] = { 0.0 };
		numtyp hidly_dw[10 * 28] = { 0.0 };																
		...
		for (int n = 0; n < ntl - 1; n++) {
			...
			for (int k = 0; k < nrc_w[n].x; k++) {
				...
				for (int m = 0; m < nrc_w[n].y; m++) {
					//fetch(weight, index_w, weight_tex);															// this one cannot be used on RTX A50000 card	
			...
			for (int k = 0; k < nrc_w[n].x; k++) {												
				for (int m = 0; m < nrc_w[n].y; m++) {
					//fetch(weight, index_w2, weight_tex);															// this one cannot be used on RTX A50000 card									
		...
		// get energy and force for atom i: 
		if (offset == 0) {
