...
PairANNPGPU::PairANNPGPU(LAMMPS *lmp) : PairANNP(lmp), gpu_mode(GPU_FORCE) {
    respa_enable = 0;                                                                               // 1 if inner/middle/outer rRESPA routines, "pair.h" file
    cpu_time = 0.0;                                              
...
void PairANNPGPU::compute(int eflag, int vflag) {
    ...
    double** f = atom->f;                                                                           // for update the total energy 
...
void PairANNPGPU::init_style() {
    ...
    double* sfnor_scal, * sfnor_avg;                                                                // for normalizing the symmetry function
    double*** weight_all, *** bias_all;                                                             // weight_all[#elements][#lyaer][value], bias_all[#elements][#lyaer][value]
    weight_all = nullptr;                                                                           // 3D, x: 10*28(nnod*nsf), y: nlayer-1, z: nelements
    bias_all = nullptr;                                                                             // 3D, x: nnod, y: nlayer-1, z: nelements
    flagact = nullptr;

    // get the parameters from potential file
    ntl = params[0].ntl;                                                                            // now, we just have one potential file
    nhl = params[0].nhl;
    nnod = params[0].nnod;
    nsf = params[0].nsf;
    npsf = params[0].npsf;
    ...
    nelements = params[0].nelements;                                                                // number of elements in potential files
    ...
    for (int nt = 1; nt <= atom->ntypes; nt++) {                                                    
        int rtype = map[nt];                                                                        // index of map start from 1
    ...
    int success = annp_gpu_init(atom->ntypes, atom->nlocal, atom->nlocal + atom->nghost,            // in "lal_annp_ext.cpp" file
